    ...
    public RouteLocator routes(RouteLocatorBuilder builder)
    {
        return builder.routes()
                .route("identity-service", r -> r.path("/identity/**")
                        .filters(f -> f.filter(filter)).uri("lb://identity-service"))
                .route("food-delivery", r -> r.path("/foodDelivery/**")
                        .filters(f -> f.filter(filter)).uri("lb://food-delivery"))
                .route("payment-service", r -> r.path("/payment/**")
                        .filters(f -> f.filter(filter)).uri("lb://payment-service"))
                .route("restaurant-info-service", r -> r.path("/restaurantInfo/**")
                        .filters(f -> f.filter(filter)).uri("lb://restaurant-info-service"))
                .route("courier-manager", r -> r.path("/courierManager/**")
                        .filters(f -> f.filter(filter)).uri("lb://courier-manager"))
                .build();
    }
}
